...
### Fixed
- **`WidgetVariableNameToGuidMap` ensure eliminated at the source on UE 5.6 (TACB-929)** — the compat gate `MCP_HAS_WIDGET_VARIABLE_GUID_MAP` excluded 5.6, so `RegisterWidgetGuid` compiled to a no-op and every widget `add_*` tripped the engine's `WidgetVariableNameToGuidMap.Contains(...)` ensure (~34 captured per call). The member is in fact public on 5.6 (`UMGEditor/Public/WidgetBlueprint.h:239`), so the gate now enables `ENGINE_MINOR_VERSION >= 6`. Additionally `SafeAddWidgetToTree` now calls `RegisterAllWidgetGuids` before the caller's structural-modify, so pre-existing widgets authored while the gate was off get registered too — clearing the ensure tree-wide, not just for the newly-added widget. Verified: a post-fix `add_text_block` returns zero `engineErrorsObserved` and the editor log shows zero ensures. `RegisterAllWidgetGuids` also now prunes stale map entries (reconcile to the live tree), and `remove_widget` unregisters the removed subtree's GUIDs. (Residual: removing a widget still emits a non-fatal, engine-self-healed compile ensure because the compiler's source-widget snapshot briefly lags the tree edit -- tracked in TACB-935 for a reconcile-before-every-structural-modify sweep; the add/style/reorder audit path is fully clean.)
- **`set_position` / `set_size` now honor top-level `x`/`y`/`width`/`height`** — the canvas-slot handlers only read a nested `{ position: {x,y} }` / `{ size: {x,y} }` object, but the tool schema exposes top-level `x`/`y`/`width`/`height`. Passing those (as the schema documents) silently no-opped: the widget never moved/resized yet the call returned `"Position set"`/`"Size set"`. Both now accept the top-level params (falling back to the nested object), echo the applied values in the response, and error `WRONG_SLOT_TYPE` on a non-canvas slot instead of silently succeeding. This is what blocked authoring any absolutely-positioned canvas layout via MCP.
- **Widget `add_*` no longer fails on a benign captured ensure** — `ValidateWidgetCreation` treated ANY globally-captured engine error (including a non-fatal "Handled ensure", e.g. the `WidgetVariableNameToGuidMap.Contains` bookkeeping ensure) as a creation failure, so every `add_image`/`add_text_block`/`add_horizontal_box`/etc. returned `ENGINE_ERROR` despite the widget being present in the tree. Presence in the widget tree is now the authoritative success signal; captured errors are only consulted (as context) when the widget is genuinely absent. The ensures still surface as non-fatal `engineErrorsObserved` in the response.
- **`add_*` into a full single-child container fails loudly** — adding a child into a `UContentWidget` (Button/Border/SizeBox) that already has a child previously orphaned the widget silently (and its later descendants failed to register, reading as a confusing "not found after creation"). `SafeAddWidgetToTree` now returns a clear error advising to add into the existing child or wrap the contents in a panel.
